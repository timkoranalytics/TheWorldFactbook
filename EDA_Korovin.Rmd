...
date: "Project 1: The World Factbook"
output:
  pdf_document:
...
Using \textit{The CIA World Factbook} it is needed to construct a data frame in which units (rows) are world countries with names from the “book” and variables (columns). Next, we should save the created data frame as a CSV file. Finally, we have to explore the collected data.
\vspace*{\fill}
\begin{center}
\emph{Creation date:} 10/03/2025

\emph{The last change date:} 21/04/2025
\end{center}
---
...
\textbf{To what extent is the accessibility of medical services (measured by physician density and hospital bed density) associated with maternal and infant mortality rates across countries?} 

\section{1.2. Studies on that topic}
...
\subsection{Maternal mortality rate}
...
\subsection{Hospital bed density}
...
\subsection{ISO, Countries, Region}
...
\subsection{Constructing Data Frame} 
...
```
```{r}
write.csv(df_countries, file = "df_countries.csv", row.names = FALSE)
```
...
df_regions <- df_countries %>%
  group_by(V6) %>%
  drop_na() %>% # 
  summarise(across(where(is.numeric), mean, na.rm = TRUE, .names = "mean{.col}")) 

print(df_regions)
```

\subsection{Distributions of our variables}
...
\subsection{Maternal Mortality rate by region}
...
df_regions1 <- df_regions %>%
  mutate(V6 = factor(V6, levels = rev(df_regions$V6[order(-df_regions$meanV2)])))

ggplot1 <- ggplot(df_regions1, aes(x=meanV2, y=V6)) +
  geom_bar(stat="identity", fill='lightblue') +
  labs(x = "Maternal Mortality rate",
       y = "Region",
       main = "Maternal Mortality rate by region") +
  theme_minimal()
...
\subsection{Infant Mortality rate by region}
...
The histogram illustrates the distribution of physician density across different countries, measured by the number of physicians per 1,000 people.

The distribution is right-skewed, with a long tail extending to the right. This indicates that a majority of countries have a relatively low physician density, while a smaller number of countries have a significantly higher density.

Most of the countries fall in in the 0-1 bin, illustrating the significant proportion of countries fails to provide an adequate access to the healthcare. 

\subsection{Physicians density  by region} 
...
\subsection{Hospital Bed density by region}
...
ggplot3
```
The graph displays the density of hospital beds across different world regions. Asia Europe has the highest density of hospital beds, exceeding 6 beds per 1,000 people. It may reflect substantial investments in healthcare infrastructure in this region.
...
\subsection{Physicians density for Africa}
...
\subsection{Hospital bed density for Africa}
...
Mostly, countries in Central and West Africa demonstrate lowest rates of the hospital bed density (less than 2 per 1000 people) within Africa region. 
